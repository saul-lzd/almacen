...
/**
 * ## ensureInitEntryFile
 * When multiLocaleSupport is enabled for an existing Webpack project,
 * ensure the init-* entry file exists by generating it from the base
 * root file. This avoids requiring users to run
 * `ojet add webpack --multi-locale` after toggling the config flag.
 */
function ensureInitEntryFile() {
  const oraclejetConfig = ojetUtils.getOraclejetConfigJson();
  const multiLocale = oraclejetConfig && oraclejetConfig.multiLocaleSupport;
  if (!multiLocale) return; // nothing to do

  // Compute the expected init file path using the same logic as getEntryFilePath
  const initFilePath = getEntryFilePath();
  if (fs.existsSync(initFilePath)) return; // already present, do not overwrite

  // Derive the base (original) root file path by stripping the init- prefix
  const dir = path.dirname(initFilePath);
  const initBaseName = path.basename(initFilePath); // e.g., init-root.ts or init-index.ts
  const baseName = initBaseName.replace(/^init-/, ''); // root.ts or index.ts/js
  const basePath = path.join(dir, baseName);

  // If the original base file doesn't exist, surface a clear, actionable error
  if (!fs.existsSync(basePath)) {
    ojetUtils.log.error(
      `Unable to locate the base root file: ${basePath}.\n` +
      'The multi-locale init entry cannot be generated because the expected source file is missing or the src/js or src/ts folder was moved or deleted.\n' +
      'Restore the file/folder or update oraclejetconfig.json paths and try again.'
    );
  }

  const fileNameWithoutExt = path.basename(basePath, path.extname(basePath));
  const isJsRoot = basePath.endsWith('root.js');
  ...
  if (!fs.existsSync(templatePath)) {
    // Template file is required; without it we do not generate the init entry.
    ojetUtils.log.error(
  ...
  content = content.replace(/__ROOT_MODULE__/g, fileNameWithoutExt);

  // Write the init file next to the base file
  ojetUtils.ensureDir(dir);
...
function getEntryObject() {
  ...
  // If multi-locale is enabled but the init entry file is missing (e.g., flag toggled manually),
  // generate it so Webpack can resolve the entry without additional user commands.
  ensureInitEntryFile();
