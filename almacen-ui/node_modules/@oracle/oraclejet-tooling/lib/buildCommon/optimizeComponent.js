...
function optimizeComponent({
  ...
  return util.runPromisesInSeries([
    ...
    () => cssPatchBeforeOptimize({ context, root }),
    () => optimizeComponentInvoker({ context, rjsOptions, tempBundleFile }),
    () => ojcssPatchAfterOptimize({ context, root }),
    () => cssPatchAfterOptimize({ context, root }),
  ]);
}

/**
 * Pre-compute a Set of importNames that come from reference components.
 * Call this once before processing, then use Set.has() for O(1) lookups.
 * @param {object} componentCache - The component cache object
 * @returns {Set<string>} Set of importNames from reference components
 */
function getReferenceComponentImportNames(componentCache) {
  const referenceImportNames = new Set();

  if (!componentCache || typeof componentCache !== 'object') {
    return referenceImportNames;
  }

  Object.values(componentCache).forEach((entry) => {
    if (entry &&
        entry.importName &&
        entry.componentJson &&
        entry.componentJson.type === CONSTANTS.COMPONENT_TYPE.REFERENCE) {
      referenceImportNames.add(entry.importName);
    }
  });
...
/**
 * Patch the r.js configuration to workaround the lack of support
 * for the empty: scheme when using the css plugin, too. We create
 * an empty css file at the same folder as the .js file using the plugin
 * which ensures that the path is resolved.
 *
 * @param {object} options.root - component root, either the component name if
 * it is a singleton or the pack name if it is in a JET pack
 * @param {object} options.context - build context
 * @returns {Promise<object>} build context
 */
async function cssPatchBeforeOptimize({ root, context }) {
  if (root) {
    const { componentRequireJs } = context.opts;
    const pathMappingKeys = Object
      .keys(componentRequireJs.paths)
      .filter(pathMappingKey => componentRequireJs.paths[pathMappingKey] === 'empty:');

    // Pre-compute reference imports ONCE before the loops
    const componentCache = util.getComponentsCache();
    const referenceImportNames = getReferenceComponentImportNames(componentCache);

    getJavascriptFilesInComponentFolder({ context })
      .forEach((filePath) => {
        let content = util.readFileSync(filePath);
        pathMappingKeys
          .forEach((pathMappingKey) => {
            const isImportFromReference = referenceImportNames.has(pathMappingKey);

            if (content.includes(`css!${pathMappingKey}`) && isImportFromReference) {
              // eslint-disable-next-line no-useless-escape
              const regex = new RegExp(`\\bcss!${pathMappingKey}(?<pathGroup>[^"'\\s]+)[\"']`, 'g');
              const match = regex.exec(content);

              if (match) {
                const cssPathGroup = match.groups && match.groups.pathGroup;
                const pathToCssFile = path.join(
                  path.dirname(filePath),
                  pathMappingKey,
                  cssPathGroup
                );
                if (!fs.existsSync(pathToCssFile)) {
                  fs.createFileSync(pathToCssFile);
                }

                content = content.replace(
                  new RegExp(`css!${pathMappingKey}`, 'g'),
                  `css!./${pathMappingKey}`
                );
              }
            }
...
/**
 * Reverts the patch done in the cssPatchBeforeOptimize function.
 * After the build passes, we delete the dummy css file and then
 * retain the original css! import.
 *
 * @param {object} options.root - component root, either the component name if
 * it is a singleton or the pack name if it is in a JET pack
 * @param {object} options.context - build context
 * @returns {Promise<object>} build context
 */
async function cssPatchAfterOptimize({ root, context }) {
  if (root) {
    const { componentRequireJs } = context.opts;
    const pathMappingKeys = Object
      .keys(componentRequireJs.paths)
    ...
    // Pre-compute reference imports ONCE
    const componentCache = util.getComponentsCache();
    const referenceImportNames = getReferenceComponentImportNames(componentCache);
      ...
      .forEach((filePath) => {
        ...
        pathMappingKeys
          .forEach((pathMappingKey) => {
            ...
            if (content.includes(`css!./${pathMappingKey}`) && isImportFromReference) {
              const pathToCssFileFolder = path.join(path.dirname(filePath), pathMappingKey);

              if (fs.existsSync(pathToCssFileFolder)) {
                fs.removeSync(pathToCssFileFolder);
              }
