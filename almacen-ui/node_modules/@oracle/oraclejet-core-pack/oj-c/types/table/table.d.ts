...
type CTableContextMenuContext<K, D, C extends string> = {
    type: 'data';
    columnKey: C | TableDefaultColumnKey;
    rowKey: K;
    rowData: D;
...
type CTableContextMenuConfig<K, D, C extends string> = {
    accessibleLabel?: string;
    items: (context: CTableContextMenuContext<K, D, C>) => Array<ContextMenuItems | 'defaultMenuItems'>;
};
export type TableProps<K extends string | number, D, C extends string> = ObservedGlobalProps<'aria-describedby' | 'aria-label' | 'aria-labelledby' | 'id'> & {
    /**
     ...
     */
    contextMenuConfig?: CTableContextMenuConfig<K, D, C>;
};
/**
 * @classdesc
 ...
 */
declare function TableImpl<K extends string | number, D, C extends string>({ 'aria-label': ariaLabel, 'aria-labelledby': ariaLabelledBy, 'aria-describedby': ariaDescribedBy, id, data, columns, row, horizontalGridVisible, verticalGridVisible, layout, currentCellOverride, onCurrentCellChanged, selected, selectionMode, onSelectedChanged, selectAllControl, columnOrder, cellTemplate, headerTemplate, footerTemplate, templates, onOjRowAction, noData, columnWidths, onColumnWidthsChanged, columnResizeBehavior, scrollPolicyOptions, contextMenuConfig }: TableProps<K, D, C>): import("preact").JSX.Element;
/**
...
export namespace CTableElement {
    ...
    type contextMenuConfigChanged<K extends string | number, D, C extends string> = JetElementCustomEventStrict<CTableElement<K, D, C>['contextMenuConfig']>;
    type currentCellChanged<K extends string | number, D, C extends string> = JetElementCustomEventStrict<Parameters<Required<TableProps<K, D, C>>['onCurrentCellChanged']>[0]>;
...
export interface CTableElementEventMap<K extends string | number, D, C extends string> extends HTMLElementEventMap {
    ...
    'contextMenuConfigChanged': JetElementCustomEventStrict<CTableElement<K, D, C>['contextMenuConfig']>;
    'currentCellChanged': JetElementCustomEventStrict<Parameters<Required<TableProps<K, D, C>>['onCurrentCellChanged']>[0]>;
...
export interface CTableElementSettableProperties<K extends string | number, D, C extends string> extends JetSettableProperties {
     ...
     * </table>
     */
    contextMenuConfig?: TableProps<K, D, C>['contextMenuConfig'];
    /**
     * The cell override to apply to the current cell of the Table.
     * In order for this property to be honored, a new object instance must be set.
     * Any or all aspects of a valid <code>DataCell</code>, <code>HeaderCell</code>, or
...
export interface TableIntrinsicProps extends Partial<Readonly<CTableElementSettableProperties<any, any, any>>>, GlobalProps, Pick<preact.JSX.HTMLAttributes, 'ref' | 'key'> {
    ...
    oncontextMenuConfigChanged?: (value: CTableElementEventMap<any, any, any>['contextMenuConfigChanged']) => void;
