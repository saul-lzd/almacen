<?js
  ...
  var methods = data.funcVCompMethodSignatures ?
                  Object.keys(data.funcVCompMethodSignatures).sort() :
                  data.methods ?
                    Object.keys(data.methods).sort() :
                    undefined;
  if (data.imports){
...
<?js
  if (data.properties) {
    ...
    if (data.propsComponentPropsAlternateName.length > 0) {
      data.propsName = data.propsComponentPropsAlternateName;
      data.typeParams = "";
    }
    else if (data.propsReadonlyMappedTypesClassName.length > 0) {
  ...
  if (methods) {
    for (let m of methods) {
      if (data.funcVCompMethodSignatures) {?>
  <?js= m ?>: <?js= data.funcVCompMethodSignatures[m] ?>;
  <?js
      } else {?>
...
for (let event in data.events) {
  ...
  const eventDetailTypeParams = data.events[event]['evnDetailTypeParams'] ?? '';
  if (eventDetailType && eventDetailType.indexOf('<') > -1 && eventDetailType.indexOf('>') > 1) {
    eventPropEvent = `'${event}': ${data.vcomponentName}.${event}${eventDetailTypeParams};`
  } ?>
  <?js= eventPropEvent ?>
<?js
}
if (props){
  for (let prop of props) {
    ...
    // JET-73600:
    // for a readOnly writeback property, let's not reference the type of the property we just added previously on the custom JetElement
    // but rather the original type of the payload of the callback function from the source. We do this so that we don't get the optional original
    // non-optional nature of the readonly property leaked here.
    if (Array.isArray(data.readOnlyProps) && data.readOnlyProps.indexOf(prop) > -1){
      // Uppercase the property name
      let uppercaseProp = prop.charAt(0).toUpperCase() + prop.slice(1);
      propEvent = `'${prop}Changed': JetElementCustomEventStrict<Parameters<Required<${data.propsName}${data.typeParams}>['on${uppercaseProp}Changed']>[0]>;`;
    } ?>
    <?js= propEvent ?>
  <?js
  }
...
if (data.properties) {
    ...
    else if (data.propsMappedTypesClassName.length > 0) {
