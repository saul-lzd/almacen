...
export namespace <?js= componentName ?> {
  ...
  for (let prop of props) {
    let propEvent = `type ${prop}Changed${this.classTypeParamsDeclaration} = JetElementCustomEventStrict<${data.vcomponentName}${data.classTypeParams}["${prop}"]>;`;
    ...
    if (Array.isArray(data.readOnlyProps) && data.readOnlyProps.indexOf(prop) > -1){
      ...
      propEvent = `type ${prop}Changed${this.classTypeParamsDeclaration} = JetElementCustomEventStrict<Parameters<Required<${data.propsName}${data.typeParams}>['on${uppercaseProp}Changed']>[0]>;`;
    }?>
    <?js= propEvent ?>
  <?js}
  }?>
