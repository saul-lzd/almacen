...
<section id="quick-nav" class="<?js= className?>">
    ...
    <div class="quick-nav-section<?js= columnCount ?>">
        ...
        <ul class="optionsSubList">
            ...
            <?js } 
                // render first the named and template slots
                let dynamicSlots = [];
                for (var i=0; i < membersObj.slots.length; i++) { 
                    let slot = membersObj.slots[i];
                    if (!slot.dynamicSlot){ ?>
                        <?js= self.getLiTag(membersObj.slots[i]) ?><a href="#<?js= membersObj.slots[i].id ?>"><?js= membersObj.slots[i].id ?></a></li>
                    <?js 
                    } 
                    else {
                        dynamicSlots.push(membersObj.slots[i]);
                    }
                } 
                // render any dynamic template slot under a generic parent
                if (dynamicSlots.length > 0){ 
                    if (!dynamicSlots[0].name.endsWith('DynamicSlot')) { ?>
                    <li><div><a href="javascript:void(0)"><b>Dynamic Slots</b></a></div>
                        <ul class="optionsInnerSubList">
                        <?js
                            dynamicSlots.forEach(slot => { ?>
                                <?js= self.getLiTag(slot) ?><div><a href="#<?js= slot.id ?>"><?js= slot.displayName || slot.name ?></a></div></li>
                            <?js
                            }); //forEach
                        ?>
                        </ul>
                <?js
                        ...
                        let slot = dynamicSlots[0]; ?>
                        <?js= self.getLiTag(slot) ?><div><a href="#<?js= slot.id ?>">Dynamic Slots</a></div></li>
                   <?js
                    }
                } ?>
        </ul>
    ...
    <div class="quick-nav-section<?js= columnCount ?>">
        ...
        <?js
        self.exportedTypes = self.typedefs.filter(td => td.ojexports) || [];
        self.docOnlyTypes = self.typedefs.filter(td => !td.ojexports) || [];
        if (self.exportedTypes){ ?>
            <h4><a href="#exported-typedefs-section">Exported Types</a></h4>
            <ul class="subList typedef">
            <?js 
            for (let i = 0; i < self.exportedTypes.length; i++) { ?>
                <?js= self.getLiTag(self.exportedTypes[i]) ?><a href="#<?js= self.exportedTypes[i].name ?>" class="exported"><?js= self.exportedTypes[i].name ?></a></li>
            <?js 
            } ?>
            </ul> 
        <?js
        }
        if (self.docOnlyTypes){ ?>
            <h4><a href="#doc-only-typedefs-section">Documentation-only Types</a></h4>
            <ul class="subList typedef">
            <?js 
            for (let i = 0; i < self.docOnlyTypes.length; i++) { ?>
                <?js= self.getLiTag(self.docOnlyTypes[i]) ?><a href="#<?js= self.docOnlyTypes[i].name ?>"><?js= self.docOnlyTypes[i].name ?></a></li>
            <?js 
            } ?>
            </ul> 
        <?js
...
<article>
    ...
    if (membersObj && (membersObj.children.length + membersObj.slots.length)) {  
        ...
        <dl>
            ...
            self.docInProgress = null;
            let dynamicSlots = [];
            // render first the non-dynamic slots, then at the end the dynamic slots
            for (var i=0; i < membersObj.slots.length; i++) {
                const slot = membersObj.slots[i];
                if (slot.dynamicSlot){
                    dynamicSlots.push(slot);
                }
                else{
                    self.docInProgress = slot;
                    memberNames = memberNames + ' ' +slot.id; 
                    ?>
                    <?js= self.partial('members.tmpl', slot) ?>
                <?js
                } 
            }
    ...
    if (self.typedefs && self.typedefs.length && self.typedefs.forEach) { ?>
        ...
        <?js
        if (self.exportedTypes && self.exportedTypes.length){ ?>
            <h4 id="exported-typedefs-section">
                Exported Types
                <a class="bookmarkable-link" title="Bookmarkable Link" href="#exported-typedefs-section"></a>
            </h4>
            <p>
                <b>Note: </b> Type definitions in this section are exported by the module, and so can be imported for use in application code.
            </p>
            <dl>
                <?js 
                const usageSampleCommentPart = '//To import this type, use the format below.<br>';
                const pathMappingPrefix = doc.pack? `${doc.pack}/` : '';
                self.exportedTypes.forEach(function(e) {
                    self.docInProgress = e;
                    const usageSample = usageSampleCommentPart + `import { ${e.id} } from "${pathMappingPrefix}${self.moduleName}";`;
                    e.usageSample = `<pre class="prettyprint"><code>`+usageSample+`</code></pre>`;
        ...
        if (self.docOnlyTypes && self.docOnlyTypes.length){ ?>
            <h4 id="doc-only-typedefs-section">
                Documentation-only Types
                <a class="bookmarkable-link" title="Bookmarkable Link" href="#doc-only-typedefs-section"></a>
            </h4>
            <p>
                <b>Note: </b> Type definitions in this section have been provided for documentation purposes and cannot be imported into application code.
            </p>
            <dl>
                <?js 
                self.docOnlyTypes.forEach(function(e) {
                    self.docInProgress = e;
                    if (e.signature || e.classsignature) { ?>
                        <?js= self.partial('method.tmpl', e) ?>
                    <?js
