...
class TableViewAdapter extends UNSAFE_TestAdapter.TestAdapter {
    /**
    ...
    async doSort(columnKey) {
        ...
        let header;
        try {
            header = await tableView.waitForElement(UNSAFE_Locators.byCss(`[data-oj-cell-type="header"][data-oj-column-key="${columnKey}"]`));
        }
        catch (_e) {
            throw new Error(`Specified column key ${columnKey} is invalid or does not exist`);
        }
        return driver.sendKeys(header, UNSAFE_Driver.Keys['ENTER']);
    }
    /**
    ...
    async doRowAction(rowKey) {
        ...
        let row;
        try {
            row = await tableView.waitForElement(UNSAFE_Locators.byCss(`[data-oj-key="${rowKey}"]`));
        }
        catch (_e) {
    ...
    async doToggleSelectRow(rowKey) {
        ...
        return this._clickItem(row, { modifiers: [UNSAFE_Driver.Keys['CONTROL_COMMAND']] });
    }
    /**
    ...
    async doToggleSelectColumn(columnKey) {
        ...
        let column;
        try {
            column = await tableView.waitForElement(UNSAFE_Locators.byCss(`[data-oj-cell-type="header"][data-oj-column-key="${columnKey}"]`));
        }
        catch (_e) {
            throw new Error(`Specified column key ${columnKey} is invalid or does not exist`);
        }
        return this._clickItem(column, { modifiers: [UNSAFE_Driver.Keys['CONTROL_COMMAND']] });
    }
    /**
    ...
    async doLoadMore() {
        const driver = this.getTestDriver();
        const tableView = await this.getElement();
        const rowCount = (await tableView.waitForElements(UNSAFE_Locators.byCss('tr[role="row"]'))).length;
        await driver.sendKeys(tableView, UNSAFE_Driver.Keys['CONTROL_COMMAND'], UNSAFE_Driver.Keys['END']);
        await UNSAFE_Driver.waitFor(async () => {
            return ((await tableView.waitForElements(UNSAFE_Locators.byCss('tr[role="row"]'))).length != rowCount ||
                (await tableView.waitForElements(UNSAFE_Locators.byCss('[data-oj-cell-type="loadMore"]'))).length === 0);
        });
    }
}

exports.TableViewAdapter = TableViewAdapter;
