...
// TODO: For future dev who is going to work in JET-77693
// The suggestion in the jira is to compute crossAxis as
// crossAxis: isRtl ? width - center.x - DATATIP_OFFSET: center.x + DATATIP_OFFSET
// For some reason, what worked for Picto is:
// crossAxis: isRtl ? width - center.x + DATATIP_OFFSET : center.x + DATATIP_OFFSET
// which is what's implemented in calculateOffsetForPicto below.
// If the calculateOffsetForPicto is the correct way, please remove calculateOffsetForPicto.
const calculateOffset = (isRtl, width, bounds, isBottomStart, height) => {
    if (!bounds) {
    ...
    return {
        mainAxis: (isBottomStart ? 1 : -1) * center.y + datatipUtils.DATATIP_OFFSET + (isBottomStart ? -1 * height : 0),
        crossAxis: isRtl ? width - center.x : center.x
    };
};
const calculateOffsetForPicto = (isRtl, width, bounds, isBottomStart, height) => {
    if (!bounds) {
        return { mainAxis: datatipUtils.DATATIP_OFFSET, crossAxis: datatipUtils.DATATIP_OFFSET };
    }
    const center = {
    ...
    return {
        mainAxis: (isBottomStart ? 1 : -1) * center.y + datatipUtils.DATATIP_OFFSET + (isBottomStart ? -1 * height : 0),
        crossAxis: isRtl ? width - center.x + datatipUtils.DATATIP_OFFSET : center.x + datatipUtils.DATATIP_OFFSET
    };
};

exports.calculateOffset = calculateOffset;
exports.calculateOffsetForPicto = calculateOffsetForPicto;
exports.getRandomId = getRandomId;
exports.getUnion = getUnion;
exports.intersects = intersects;
//# sourceMappingURL=layoutUtils-af53a104.js.map
