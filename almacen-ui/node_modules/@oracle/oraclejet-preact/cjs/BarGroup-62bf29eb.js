...
function Bar(props) {
    ...
    return (jsxRuntime.jsx("rect", { x: props.x, y: props.y, "data-oj-private-series-index": props.seriesIndex, "data-oj-private-group-index": props.groupIndex, "data-oj-private-object": "bar", "aria-label": props['aria-label'], role: props['aria-label'] ? 'img' : undefined, id: props.activeId, style: {
            width: props.width,
            height: props.height,
            fill: props.fill,
            stroke: props.stroke,
...
function InteractiveBar(props) {
    ...
    return (jsxRuntime.jsxs("g", { id: props.activeId, "data-oj-private-object": "bar", "data-oj-private-series-index": props.seriesIndex, "data-oj-private-group-index": props.groupIndex, className: props.isDimmed ? PRIVATE_Chart_themes_ChartStyles_css.styles.dimmed : '', role: props['aria-label'] ? 'img' : undefined, "aria-label": props['aria-label'], children: [jsxRuntime.jsx("rect", { x: x, y: y, class: classNames.classNames([isSelected ? PRIVATE_Chart_themes_ChartStyles_css.styles.selectedBorder : '']), style: {
                    width: width,
                    height: height,
...
function getBarPosition(series, groups, getDataItem, xScale, yScale, y2Scale, hiddenIds, dataItemGaps, axisStepWidth, averageGroupZ, barGapRatio, isLog, isStacked, isRtl, isHoriz, isSplitDualY, getStackGroup, timeAxisType) {
    ...
    const stackGroupOf = (s) => (getStackGroup?.(s) ?? 'default');
    const yStackGroups = Array.from(new Set(seriesY.map(stackGroupOf)));
    const y2StackGroups = Array.from(new Set(seriesY2.map(stackGroupOf)));
    const numStacksY = yStackGroups.length;
    const numStacksY2 = y2StackGroups.length;
    ...
    return (seriesIndex, groupIndex) => {
        ...
        for (let index = 0; index < numOfSeries; index += 1) {
            const currSeries = series[index];
            const item = getDataItem(index, groupIndex);
            if (!item || hiddenIds?.has(item.id) || (isLog && item.value <= 0)) {
                continue;
            }
            const isCurrY2 = currSeries.associatedYAxis === 'y2';
            const axis = isCurrY2 ? 'y2' : 'y';
            const stackGroup = stackGroupOf(currSeries);
            const key = `${stackGroup}_${axis}`;
            if (!cumuls.has(key)) {
                cumuls.set(key, {
                    pos: isLog ? (isCurrY2 ? y2Scale : yScale).domain()[0] : 0,
                    neg: 0
                });
            }
            // Grouped-stacked layout: treat each stack group as a side-by-side "series"
            const useGroupedStacks = isStacked && (isCurrY2 ? numStacksY2 : numStacksY) > 1;
            const gapsSeriesIndex = useGroupedStacks
                ? isCurrY2
                    ? y2StackGroups.indexOf(stackGroup)
                    : yStackGroups.indexOf(stackGroup)
                : isSplitDualY
                    ? (isCurrY2 ? seriesY2 : seriesY).findIndex((s) => s.id === currSeries.id)
                    : index;
            const gapsNumSeriesY = useGroupedStacks ? numStacksY : numSeriesY;
            const gapsNumSeriesY2 = useGroupedStacks ? numStacksY2 : numSeriesY2;
            // For grouped-stacked, compute side-by-side offsets (not stacked) for bar gaps
            const gapsIsStacked = isStacked && !useGroupedStacks;
            const { barWidth, offset, itemsGaps } = utils.getBarGapsInfo(gapsSeriesIndex, groupIndex, currSeries, axisStepWidth, averageGroupZ, getDataItem, barGapRatio, isSplitDualY
                ? gapsNumSeriesY
                : useGroupedStacks
                    ? isCurrY2
                        ? numStacksY2
                        : numStacksY
                    : series.length, isSplitDualY
                ? gapsNumSeriesY2
                : useGroupedStacks
                    ? isCurrY2
                        ? numStacksY2
                        : numStacksY
                    : series.length, dataItemGaps, gapsIsStacked, isRtl, isHoriz);
            if (seriesIndex === index) {
                // Compute group center following BarGroup logic
                let groupCenter;
                if (timeAxisType === 'mixedFrequency') {
                    groupCenter = utils.getMixedFreqGroupCenter(xScale, item);
                }
                else {
                    let xVal = groupIndex;
                    if (timeAxisType) {
                        xVal = new Date(groups[groupIndex].id).getTime();
                    }
                    groupCenter = xScale.transform(xVal);
                }
                const x1 = groupCenter + offset;
                const x2 = x1 + barWidth;
                const cumul = cumuls.get(key);
                const negCumul = isStacked
                    ? cumul.neg
                    : isCurrY2
                        ? y2NegCumulativeValue
                        : yNegCumulativeValue;
                const posCumul = isStacked
                    ? cumul.pos
                    : isCurrY2
                        ? y2PosCumulativeValue
                        : yPosCumulativeValue;
                const { baselineCoord, endCoord } = getBaseAndEndCoord(item.value, isCurrY2 ? y2Scale : yScale, isStacked, isLog, itemsGaps, negCumul, posCumul);
                return getRectCoord(x1, x2, endCoord, baselineCoord, isHoriz);
            }
            // Update cumulatives for subsequent series
            if (isStacked) {
                const cumul = cumuls.get(key);
                if (item.value < 0) {
...
function getSeriesOffsetAndWidth(axisStepWidth, averageGroupZ, series, getDataItem, barGapRatio, dataItemGaps, isStacked, isRtl, isHoriz) {
    ...
    return (seriesIndex, groupIndex) => {
        const { offset, barWidth } = utils.getBarGapsInfo(seriesIndex, groupIndex, series[seriesIndex], axisStepWidth, averageGroupZ, getDataItem, barGapRatio, numSeriesY, numSeriesY2, dataItemGaps, isStacked, isRtl, isHoriz);
...
function BarGroup({ yScale, y2Scale, xScale, colors, series, groupIndex, hiddenIds, barGapRatio, dataItemGaps, axisStepWidth, averageGroupZ, highlightedIds, selectedIds, isPointInsideMarquee, orientation, isLog, activeId, getDataItem, isRtl, isStacked, isSelectionEnabled, focusedItemInfo, hoveredItemInfo, isDrillEnabled, group, timeAxisType, getTextDimensions, defaultFontSize, stacklabelStyle, getStackLabel, getStackGroup, getItemAriaLabel, splitDualY, seriesIndex, plotAreaClipPathId }) {
    ...
    const numSeriesY = seriesY.length;
    const numSeriesY2 = series.length - numSeriesY;
    // Determine unique stack groups per axis (preserving series order)
    const stackGroupOf = (s) => (getStackGroup?.(s) ?? 'default');
    const yStackGroups = Array.from(new Set(seriesY.map(stackGroupOf)));
    const y2StackGroups = Array.from(new Set(seriesY2.map(stackGroupOf)));
    const numStacksY = yStackGroups.length;
    ...
    const processSeries = (_seriesIndex) => {
        ...
        const isFocused = focusedItemInfo?.refObjId == undefined &&
        ...
        const stackGroup = getStackGroup?.(series[_seriesIndex]) ?? 'default';
        const axis = isAssociatedToY2 ? 'y2' : 'y';
        const key = stackGroup + '_' + axis;
        if (!cumuls.has(key)) {
            cumuls.set(key, {
                pos: isLog ? (isAssociatedToY2 ? y2Scale : yScale).domain()[0] : 0,
                neg: 0,
                center: 0
            });
        }
        if (!posItemsMap.has(key)) {
            posItemsMap.set(key, []);
            negItemsMap.set(key, []);
        }
        const cumul = cumuls.get(key);
        ...
        const posCumul = isStacked
            ...
            : isAssociatedToY2
                ? y2PosCumulativeValue
                : yPosCumulativeValue;
        // Choose index and counts for bar layout based on stack groups.
        // When there are multiple stack groups on the same axis, we treat each stack group
        // as a separate "series" for horizontal positioning, so grouped stacks render side-by-side.
        const stackGroupIndex = (axis === 'y2' ? y2StackGroups : yStackGroups).indexOf(stackGroup);
        const useGroupedStacks = isStacked && (axis === 'y2' ? numStacksY2 : numStacksY) > 1;
        const gapsSeriesIndex = useGroupedStacks
            ? stackGroupIndex
            : splitDualY
        ...
        const gapsNumSeriesY = useGroupedStacks ? numStacksY : numSeriesY;
        const gapsNumSeriesY2 = useGroupedStacks ? numStacksY2 : numSeriesY2;
        // For grouped-stacked layout, pass isStacked=false to compute side-by-side offsets for stacks.
        const gapsIsStacked = isStacked && !useGroupedStacks;
        const { barWidth, offset, itemsGaps } = utils.getBarGapsInfo(gapsSeriesIndex, groupIndex, series[_seriesIndex], axisStepWidth, averageGroupZ, getDataItem, barGapRatio, splitDualY
            ? gapsNumSeriesY
            : useGroupedStacks
                ? axis === 'y2'
                    ? numStacksY2
                    : numStacksY
                : series.length, splitDualY
            ? gapsNumSeriesY2
            : useGroupedStacks
                ? axis === 'y2'
                    ? numStacksY2
        ...
        const x1 = groupCenter + offset;
        const x2 = x1 + barWidth;
        cumuls.get(key).center = (x1 + x2) / 2;
        const { baselineCoord, endCoord } = getBaseAndEndCoord(item.value, isAssociatedToY2 ? y2Scale : yScale, isStacked, isLog, itemsGaps, negCumul, posCumul);
        if (isStacked) {
            if (item.value < 0) {
                cumul.neg += item.value;
            }
        ...
        if (isStackLabelRendered) {
            if (item.value >= 0) {
                posItemsMap.get(key).push(item);
            }
            else {
                negItemsMap.get(key).push(item);
            }
        }
        ...
        // when value is zero, the bar will be transparent and of height 3px
        // parity check with legacy, check behaivour during stacking.
        if (Math.abs(endCoord - baselineCoord) < 1) {
            barprops.fill = 'transparent';
            if (!isHoriz) {
                rectParam.height = ZERO_BAR_SIZE;
                if (rectParam.y > baselineCoord || yScale.domain()[1] <= 0) {
                    // if horizontal, R2L must be considered to draw bar on positive side of baseline
                    rectParam.y = baselineCoord;
                }
                else {
                    rectParam.y = baselineCoord - ZERO_BAR_SIZE;
                }
            }
            else {
                rectParam.width = ZERO_BAR_SIZE;
                rectParam.x = rectParam.x - ZERO_BAR_SIZE;
            }
        }
        ...
        const ariaLabel = accUtils.supportsMobileScreenReader || isActive
            ...
            : '';
        items.push(preact.createElement(Bar, { ...rectParam, ...barprops, seriesIndex: _seriesIndex, groupIndex: groupIndex, isFocused: isFocusVisible, activeId: isActive ? activeId : '', "aria-label": ariaLabel, isDimmed: !isHighlighted, isSelected: !!isSelected, isPointerEnabled: isDrillable || isSelectionEnabled, key: `${groupIndex};${_seriesIndex}` }));
        labels.push(jsxRuntime.jsx(BarLabel, { item: item, isRtl: isRtl, seriesColor: colors[_seriesIndex % 12], isStacked: isStacked, defaultFontSize: defaultFontSize, barDim: rectParam, isHoriz: isHoriz, getTextDimensions: getTextDimensions }));
    };
    ...
    return (jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [jsxRuntime.jsx("g", { "clip-path": plotAreaClipPathId, children: items }), labels, isStackLabelRendered &&
                getTextDimensions &&
                [...cumuls.keys()].map((key) => {
                    const cumul = cumuls.get(key);
                    const axis = key.split('_')[1];
                    const yScaleToUse = axis === 'y2' ? y2Scale : yScale;
                    return (jsxRuntime.jsx(StackLabels, { group: group, isRtl: isRtl, yScale: yScaleToUse, getStackLabel: getStackLabel, defaultFontSize: defaultFontSize, getTextDimensions: getTextDimensions, posItemsData: posItemsMap.get(key), negItemsData: negItemsMap.get(key), isHoriz: isHoriz, groupCenter: cumul.center, labelStyle: stacklabelStyle, posValue: cumul.pos, negValue: cumul.neg }, key));
                })] }));
}

exports.BarGroup = BarGroup;
exports.getBarCenterCoordinate = getBarCenterCoordinate;
exports.getBarPosition = getBarPosition;
exports.getSeriesOffsetAndWidth = getSeriesOffsetAndWidth;
//# sourceMappingURL=BarGroup-62bf29eb.js.map
