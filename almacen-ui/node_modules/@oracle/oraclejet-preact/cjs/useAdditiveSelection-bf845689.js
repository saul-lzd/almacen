...
function useAdditiveSelection(keyExtractor, selectedKeys, selectionMode, isSelectionRequired, getIndexByKey, getKeysByRange, getPrevNextKey, currentKey, onChange, onRangeChange, allowEnterKey = true) {
    ...
    const cursorKey = hooks.useRef();
    // shift+click on checkbox vs item behave differently, use a pending start key to help
    const pendingAnchorKey = hooks.useRef();
    // reset anchorKey and cursorKey when a new range happen (e.g. after arrow key, click on item/selector)
    const setAnchorAndCursorKey = hooks.useCallback((key) => {
        anchorKey.current = key;
        cursorKey.current = undefined;
        pendingAnchorKey.current = undefined;
    }, []);
    // need to reset anchorKey and cursorKey during capture phase, because Selector doesn't propagate click event
    const onClickCapture = hooks.useCallback((event) => {
        if (selectionMode === 'multiple') {
            const itemKey = preClickProcessing(event, keyExtractor);
            if (itemKey === undefined)
                return;
            if (!event.shiftKey || anchorKey.current === undefined) {
                // reset anchorKey and cursorKey, a new range could happen after the click
                setAnchorAndCursorKey(itemKey);
            }
            else {
                // shift+click on item should not update anchor key, shift+click on checkbox should update anchor key
                if (pendingAnchorKey.current !== undefined) {
                    setAnchorAndCursorKey(pendingAnchorKey.current);
                }
                pendingAnchorKey.current = itemKey;
            }
        }
    }, [keyExtractor, selectionMode, setAnchorAndCursorKey]);
    const onClick = hooks.useCallback((event) => {
        ...
        if (selectionMode === 'single') {
            ...
            if (!event.shiftKey || anchorKey.current === undefined) {
                ...
                // shift+click on item should not update anchor key, clear the pending anchor key here
                pendingAnchorKey.current = undefined;
    ...
    const onKeyDown = hooks.useCallback((event) => {
        ...
        if (selectionMode === 'multiple') {
            ...
            if (event.ctrlKey || event.metaKey) {
                setAnchorAndCursorKey(currentKey);
                return;
            }
    ...
    const additiveSelectionProps = selectionMode === 'none' || onChange == null
        ? {}
        : { onClickCapture, onClick, onKeyDown, onKeyUp, onPointerDown };
    return {
        eventProps: additiveSelectionProps
    };
}
/**
 * The helper function to determine whether we should process the click or click capture event.
 * Only process when there is a valid itemKey. Return the itemKey in this case.
 * @param event
 * @param keyExtractor
 */
const preClickProcessing = (event, keyExtractor) => {
...
const toggleSelection = (event, selectedKeys, selectionMode, isSelectionRequired, itemKey, onChange) => {
    let keySet = selectedKeys;
    const selected = keys.containsKey(keySet, itemKey);
    if (!selected) {
        if (selectionMode === 'single') {
            keySet = { all: false, keys: new Set([itemKey]) };
        }
        else {
            keySet = keys.addKey(keySet, itemKey);
        }
    }
    else {
        keySet = keys.removeKey(keySet, itemKey, isSelectionRequired);
    }
    if (selectedKeys != keySet && onChange) {
        onChange({ value: keySet, target: event.target });
        if (event.type === 'click') {
...
exports.useAdditiveSelection = useAdditiveSelection;
//# sourceMappingURL=useAdditiveSelection-bf845689.js.map
