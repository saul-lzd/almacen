...
var utils$1 = require('./utils-c0efcd5e.js');
...
function getTimeAxisProps(groups, position, availSpace, viewportMin, viewportMax, averageInterval, isRtl, xAxis, axisFontSize, getTextDimensions, isTimeAxis) {
    ...
    if (isLabelRendered) {
        ...
        isLabel2Rendered = isHoriz
            ...
            : level2AxisDim < axisMaxDim - titleHeight;
        if (isLabel1Rendered) {
            const label1MaxW = labelUtils.getMaxWidth(isSize, position, axisMaxDim, label1MaxWidth, titleDim);
            const label1MaxH = labelUtils.getMaxHeight(isSize, position, axisMaxDim, label1MaxHeight, titleDim);
            const getAxisPos = (index) => {
                return labelInfos1[index].coord;
            };
            labelUtils.updateLabelsDims(label1MaxW, label1MaxH, tickLabels1, getAxisPos, availSpace, position);
        ...
        if (isLabel2Rendered) {
            const label2MaxW = labelUtils.getMaxWidth(isSize, position, axisMaxDim, label2MaxWidth, titleDim);
            const label2MaxH = labelUtils.getMaxHeight(isSize, position, axisMaxDim, label2MaxHeight, titleDim);
            const getAxisPos = (index) => {
                return labelInfos2[index].coord;
            };
            labelUtils.updateLabelsDims(label2MaxW, label2MaxH, tickLabels2, getAxisPos, availSpace, position);
...
function getDataAxisProps(position, availSpace, isRtl, locale, dataAxisInfo, dataAxis, axisFontSize, getTextDimensions, isAxisRendered) {
    ...
    let labelDims = 0;
    if (isLabelRendered) {
        const maxW = labelUtils.getMaxWidth(isSize, position, axisMaxDim, maxWidth, titleDim);
        const maxH = labelUtils.getMaxHeight(isSize, position, axisMaxDim, maxHeight, titleDim);
        labelUtils.updateLabelsDims(maxW, maxH, tickLabels, getAxisPos, availSpace, position);
        labelDims = isHoriz ? maxH + labelUtils.AXIS_DEFAULTS.labelGap : maxW + labelUtils.AXIS_DEFAULTS.labelGap;
    }
...
function getGroupAxisProps(position, availSpace, isRtl, viewportMin, viewportMax, min, max, startIndex, endIndex, isDomainFlipped, levelsArray, endOffset, barWidths, isStacked, groupAxis, axisFontSize, getTextDimensions, isTestEnv, groupCount, hasWidthWeightValue) {
    ...
    for (let level = 0; level < levelsArray.length; level++) {
        ...
        if (isLabelRendered && isLevelRendered) {
            ...
            const maxH = labelUtils.getMaxHeight(isSize, position, axisMaxDim, maxHeight, titleDim);
            labelUtils.updateLabelsDims(maxW, maxH, tickLabels, getAxisPos, availSpace, position);
            const levelDims = isHoriz ? maxH + labelUtils.AXIS_DEFAULTS.labelGap : maxW + labelUtils.AXIS_DEFAULTS.labelGap;
            labelDims += levelDims;
...
exports.getActiveInfo = getActiveInfo;
...
exports.hasInvalidData = hasInvalidData;
...
//# sourceMappingURL=utils-ee4fc356.js.map
