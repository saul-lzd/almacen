...
function startAnimation(animationConfig, animatedElement, animationState, onAnimationEnd) {
    ...
    // We call this if animation is finished (not paused or cancelled) and onfinish event was not called for any reason, e.g. opening a new tab on a testing enviroment
    const timeoutAfterAnimation = setTimeout(() => {
        if (startedAnimation.playState === 'finished') {
            finishAnimation();
        }
    }, (delay || 0) + (duration || 0) + ANIMATION_ADDITIONAL_DELAY);
    startedAnimation.onfinish = () => {
        clearTimeout(timeoutAfterAnimation);
        finishAnimation();
    };
...
const ANIMATION_ADDITIONAL_DELAY = 100;

exports.useAnimation = useAnimation;
//# sourceMappingURL=useAnimation-65c0e455.js.map
