...
class InputTimeMaskAdapter extends UNSAFE_TestAdapter.TestAdapter {
    /**
    ...
    /**
     * Clears the entire time, then changes the InputTimeMask segments within the root element, and commit the value.
    ...
    async changeValue(value) {
        ...
        const hourSegment = await el.waitForElement(UNSAFE_Locators.byCss(`div[data-segment="hour"]`));
        // Select all, then delete to clear the entire time.
        await driver.sendKeys(hourSegment, UNSAFE_Driver.Keys['CONTROL_COMMAND'], 'a');
        await driver.sendKeys(hourSegment, UNSAFE_Driver.Keys['DELETE']);
        if (value && value.hour !== undefined) {
            ...
            try {
                dayPeriodSegment = await el.waitForElement(UNSAFE_Locators.byCss(`div[data-segment="dayPeriod"]`));
            }
            catch (_) {
            ...
            if (dayPeriodSegment) {
                if (value.hour < 12) {
                    ...
                    // HOME goes to AM
                    await driver.sendKeys(dayPeriodSegment, UNSAFE_Driver.Keys['HOME']);
                }
                else {
                    ...
                    // END goes to PM
                    await driver.sendKeys(dayPeriodSegment, UNSAFE_Driver.Keys['END']);
                }
            }
            else {
        ...
        if (value && value.minute !== undefined) {
            ...
            const minuteSegment = await el.waitForElement(UNSAFE_Locators.byCss(`div[data-segment="minute"]`));
        ...
        if (value && value.second !== undefined) {
            ...
            try {
                const secondSegment = await el.waitForElement(UNSAFE_Locators.byCss(`div[data-segment="second"]`));
        ...
        if (value && value.millisecond !== undefined) {
            ...
            try {
                const millisecondSegment = await el.waitForElement(UNSAFE_Locators.byCss(`div[data-segment="millisecond"]`));
        ...
        await driver.sendKeys(hourSegment, UNSAFE_Driver.Keys['ENTER']);
    }
}

exports.InputTimeMaskAdapter = InputTimeMaskAdapter;
