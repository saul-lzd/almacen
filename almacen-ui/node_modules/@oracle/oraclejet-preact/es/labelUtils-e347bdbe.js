...
import { i as isWidthOverlap, a as isHeightOverlap } from './textUtils-3ed25dd1.js';
 ...
 * @returns
 */
function getEnclosingRectDims(dims, axisPosition, isRtl) {
    let x;
    let y;
 ...
 * @returns
 */
function getTickLabelInfo(coord, label, indexAtLevel, labelStyle, isHoriz, isRotated, isWrapAllowed, allowTruncate, maxWidth, maxHeight, availSpace, getTextDimensions) {
    ...
    if (allowTruncate || isWrapAllowed) {
        const textInfo = getTextInfo(label, labelStyle, availWidth, mxHeight, isWrapAllowed, 1, getTextDimensions);
...
function getMaxWidth(isSize, position, axisMaxDim, maxWidth, titleDim) {
    return isSize && (position === 'left' || position === 'right')
        ? axisMaxDim - (maxWidth + titleDim)
        : maxWidth;
}
function getMaxHeight(isSize, position, axisMaxDim, maxHeight, titleDim) {
    return isSize && (position === 'top' || position === 'bottom')
        ? axisMaxDim - (maxHeight + titleDim)
        : maxHeight;
}

export { AXIS_DEFAULTS as A, getTickLabelInfo as a, getMaxWidth as b, getMaxHeight as c, getDataAxisLabelsInfo as d, getEnclosingRectDims as e, getGroupAxisLabelsInfo as g, skipLabelsGreedy as s, updateLabelsDims as u };
//# sourceMappingURL=labelUtils-e347bdbe.js.map
