...
import { render, queryByAttribute, queryAllByAttribute, screen, fireEvent, waitFor } from '@testing-library/preact';
import { m as matchTranslationBundle } from '../matchTranslationBundle-d9e64706.js';
import '../LayerHost-7931b424.js';
import { R as RootEnvironmentProvider } from '../EnvironmentProvider-6a0a8212.js';
...
queryByAttribute.bind(null, 'data-oj-private-item-index');
...
function checkStringContent(content) {
    const targetElements = document.querySelectorAll('[role="tooltip"]');
    const targetElementItem = targetElements.item(0);
    expect(Array.from(targetElementItem.classList).some((className) => className.includes('useDatatip_visDatatipStyle'))).to.equal(true);
    expect(targetElementItem?.innerHTML).to.equal(content);
}
...
export { checkStringContent, checkTooltipText, setupTabbableComponent as default, getEventCoords, getTestContextMenuForNoData, getTestContextMenuViaKeyboard, getTestContextMenuViaMouse, mockMouseEventPageXYOffsetXY, queryAllBy, queryById, restoreMouseEventPageXYOffsetXY };
