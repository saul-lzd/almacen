...
const useContextMenuGesture = (onContextMenuHandler, contextMenuOptions = {
    ...
    const touchHoldGestureInfo = useRef({ isTouchHoldComplete: false });
    ...
    }, [touchGestureType, pressHoldTime, allowBrowserContextMenu]);
    const onContextMenu = useCallback((event) => {
        if (event.pointerType === 'touch' && touchGestureType === 'touchHoldRelease') {
            event.preventDefault();
            return;
        }
        if (!getIsSelectionPending$1() || !allowBrowserContextMenu) {
    ...
    }, [onContextMenuHandler, getIsSelectionPending$1, touchGestureType, allowBrowserContextMenu]);
    ...
    const onTouchStart = useCallback((event) => {
        ...
        if (event.touches.length === 1) {
            let delay = touchGestureType === 'touchHold' && allowBrowserContextMenu
    ...
    }, [onContextMenuHandler, touchGestureType, pressHoldTime, allowBrowserContextMenu]);
    ...
    const onTouchEnd = useCallback((e) => {
        ...
        if (touchHoldGestureInfo.current.isMenuOpened && allowBrowserContextMenu) {
            //Prevent default so we don't open browser context menu
            e.preventDefault();
        ...
        touchHoldGestureInfo.current.isTouchHoldComplete = false;
    }, [resetUserSelect, touchGestureType, allowBrowserContextMenu]);
    const onTouchCancel = useCallback(() => {
        clearTimeout(timerId.current);
        if (touchHoldGestureInfo.current.isMenuOpened && allowBrowserContextMenu) {
            resetUserSelect();
        }
    }, [resetUserSelect, allowBrowserContextMenu]);
...
//# sourceMappingURL=useContextMenuGesture-e09f4c69.js.map
