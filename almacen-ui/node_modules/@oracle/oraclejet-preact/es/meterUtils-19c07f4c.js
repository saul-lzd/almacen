 ...
 */
function validateRange(min, max, value, step, isRating, isReadonly, baseline) {
    const minStr = isRating ? 'zero' : 'min value';
    if (min > max) {
        throw new RangeError(`The max value must be greater or equal to ${minStr}.`);
    }
    if (value < min || value > max) {
        throw new RangeError(`The value must be between ${minStr} and max value.`);
    }
    if (!isReadonly && step > max - min) {
        throw new RangeError(`The step value must be less than the difference of max value and ${minStr}.`);
    }
    if (baseline !== undefined) {
        if (baseline < min || baseline > max) {
            throw new RangeError(`Baseline is outside ${minStr} and max value.`);
        }
    }
}
/**
...
const adjustForStep = (value, step, min, max) => {
    if (step && step > 0) {
        ...
        const roundedStepNum = Math.round(stepNum * 10) / 10; // Round to 1 decimal.
        if (roundedStepNum < 0.5) {
            return min;
        }
        const adjustedValue = Math.ceil(roundedStepNum) * step;
        return Math.max(Math.min(max, adjustedValue), min);
    }
    return value;
};

export { adjustForStep as a, getTrackAndIndicatorColor as b, getMaxRefLabelDims as c, getDefaultReferenceLineLabelFontSize as d, getThresholdColorFromValue as e, findThreshold as f, getMeterAriaProps as g, hasLabelledReferenceLine as h, validateRange as v };
//# sourceMappingURL=meterUtils-19c07f4c.js.map
