...
import { g as getLocale } from './getLocale-1c41e86a.js';
import { l } from './stringUtils-0fe25200.js';
import { A as AXIS_DEFAULTS, g as getGroupAxisLabelsInfo, s as skipLabelsGreedy, u as updateLabelsDims, a as getTickLabelInfo, b as getMaxWidth, c as getMaxHeight, d as getDataAxisLabelsInfo } from './labelUtils-e347bdbe.js';
import './datatipUtils-0a36125e.js';
import { g as getSectionDims, c as cloneDimension } from './dimensionUtils-6d5dac30.js';
import { g as getTextInfo, p as positionMultilineText } from './utils-49903d04.js';
import { S as ScaleLog, a as ScaleLinear, V as VariableWidthScale } from './scale-9a834124.js';
 ...
 * @returns
 */
function getAxisMaxSize(availSpace, isHoriz, axisSize, axisMaxSize) {
    const size = Math.max(0, Math.min(1, getAxisSize(axisSize, axisMaxSize)));
...
function getAxisSize(axisSize, axisMaxSize) {
    if (axisSize && axisMaxSize) {
        return axisSize < axisMaxSize ? axisSize : axisMaxSize;
    }
    return axisSize ?? axisMaxSize ?? AXIS_MAX_RATIO;
}
...
function getTitleProps(title, titleStyle, maxHeight, availSpace, position, isRtl, getTextDimensions) {
    ...
    const { lines, height, width, dims, lineHeight, isTruncated } = getTextInfo(title, titleStyle, maxWidth, maxHeight, true, 1, getTextDimensions);
...
function applyInitialZooming(params) {
    const { zoomAndScroll, initialZooming, currentMin, currentMax, globalMin, globalMax, startOffset, endOffset, numGroups, availSpace, isHoriz, getTextDimensions, tickLabelStyle } = params;
    const axisWidth = isHoriz ? availSpace?.height : availSpace?.width;
    const fontDims = getTextDimensions?.('MW', tickLabelStyle);
    const fontH = fontDims?.height ?? 0;
    const maxNumGroups = Math.floor((axisWidth ?? 0) / (2 * (fontH || 1))) + startOffset;
    const maxViewportSize = numGroups > 0 ? (maxNumGroups / numGroups) * (globalMax - globalMin) : 0;
    let min = currentMin;
    let max = currentMax;
    if (zoomAndScroll === 'live' && (min == null || max == null)) {
        if (initialZooming === 'first') {
            min = min ?? globalMin + startOffset;
            max = max ?? Math.min(min + maxViewportSize, globalMax + endOffset);
        }
        else if (initialZooming === 'last') {
            max = max ?? globalMax + endOffset;
            min = min ?? Math.max(max - maxViewportSize, globalMin + startOffset);
        }
    }
    return { min, max };
}
...
function getTimeAxisProps(groups, position, availSpace, viewportMin, viewportMax, averageInterval, isRtl, xAxis, axisFontSize, getTextDimensions, isTimeAxis) {
    const isHoriz = position === 'top' || position === 'bottom';
    const isSize = xAxis?.size !== undefined;
    const axisMaxDim = getAxisMaxSize(availSpace, isHoriz, xAxis?.size, xAxis?.maxSize);
    ...
    if (isLabelRendered) {
        ...
        const getLabel1Dims = (label, index) => {
            ...
            return getTickLabelInfo(rangeValue, label, index, tickLabelStyle, isHoriz, false, false, false, Infinity, Infinity, availSpace, getTextDimensions);
        };
        ...
        const { maxWidth: label1MaxWidth, maxHeight: label1MaxHeight, tickLabels: tickLabels1 } = getGroupAxisLabelsInfo(labelInfos1.map((i) => i.text), getLabel1Dims, isHoriz, false, false, numOfSafeSkips);
        axisDim = isHoriz ? label1MaxHeight : label1MaxWidth;
        isLabel1Rendered =
            isTitleRendered && isHoriz
                ? axisDim < axisMaxDim - titleHeight
                : axisDim < axisMaxDim - titleHeight;
        const { maxWidth: label2MaxWidth, maxHeight: label2MaxHeight, tickLabels: tickLabels2 } = skipLabelsGreedy(labelInfos2.map((i) => i.label2), getLabel2Dims, isHoriz);
        const level2AxisDim = isHoriz ? axisDim + label2MaxHeight : axisDim + label2MaxWidth;
        isLabel2Rendered = isHoriz
            ? level2AxisDim < axisMaxDim - titleHeight
            : level2AxisDim < axisMaxDim - titleHeight;
        if (isLabel1Rendered) {
            const label1MaxW = getMaxWidth(isSize, position, axisMaxDim, label1MaxWidth, titleDim);
            const label1MaxH = getMaxHeight(isSize, position, axisMaxDim, label1MaxHeight, titleDim);
            const getAxisPos = (index) => {
                return labelInfos1[index].coord;
            };
            updateLabelsDims(label1MaxW, label1MaxH, tickLabels1, getAxisPos, availSpace, position);
            tickLabels = tickLabels.concat(tickLabels1);
            const label1Dims = isHoriz ? label1MaxH : label1MaxW;
            axisDim = axisDim + titleHeight + label1Dims;
        }
        if (isLabel2Rendered) {
            const label2MaxW = getMaxWidth(isSize, position, axisMaxDim, label2MaxWidth, titleDim);
            const label2MaxH = getMaxHeight(isSize, position, axisMaxDim, label2MaxHeight, titleDim);
            const getAxisPos = (index) => {
                return labelInfos2[index].coord;
            };
            updateLabelsDims(label2MaxW, label2MaxH, tickLabels2, getAxisPos, availSpace, position);
            ...
            const label2Dims = isHoriz ? label2MaxH : label2MaxW;
            axisDim = level2AxisDim + titleHeight + label2Dims;
        }
    }
 ...
 * @returns
 */
function getTimeAxisInfo(groups, xAxis = {}, startOffset, endOffset, timeAxisRange, isHoriz, availSpace, zoomAndScroll, initialZooming, getTextDimensions) {
    ...
    const xAxisFontStyle = { ...xAxis?.tickLabel?.style };
    const { min, max } = applyInitialZooming({
        zoomAndScroll,
        initialZooming,
        currentMin: minValue,
        currentMax: maxValue,
 ...
 * @returns
 */
function getDataMinMax(series, groups, xStartIndex, xEndIndex, getDataItem, isStacked, isLog, hiddenSet, hideAndShowBehavior, getStackGroup) {
    ...
    const stackGroupMinMap = new Map();
    const stackGroupMaxMap = new Map();
    for (let groupIndex = 0; groupIndex < groups.length; groupIndex++) {
        const stackCumuls = new Map();
        series.forEach((chartSeries, seriesIndex) => {
            ...
            if (itemHasWidthWeightValue && item?.widthWeight != null) {
            ...
            if (groupIndex >= xStartIndex && groupIndex <= xEndIndex) {
                ...
                if (isStacked) {
                    const stackGroup = getStackGroup?.(chartSeries);
                    const axis = isAssociatedToY2 ? 'y2' : 'y';
                    const key = stackGroup + '_' + axis;
                    if (!stackCumuls.has(key)) {
                        stackCumuls.set(key, { pos: 0, neg: 0 });
                    }
                    const cumul = stackCumuls.get(key);
                    if (itemHasValue && item.value !== undefined) {
                        if (item.value > 0) {
                            cumul.pos += item.value;
                        }
                        else {
                            cumul.neg += item.value;
                        }
                    }
                }
                else {
        ...
        if (isStacked && groupIndex >= xStartIndex && groupIndex <= xEndIndex) {
            for (const [key, cumul] of stackCumuls) {
                const groupMin = isLog ? cumul.pos : cumul.neg;
                const groupMax = cumul.pos;
                if (!stackGroupMinMap.has(key)) {
                    stackGroupMinMap.set(key, Number.MAX_VALUE);
                    stackGroupMaxMap.set(key, -Number.MAX_VALUE);
                }
                stackGroupMinMap.set(key, Math.min(stackGroupMinMap.get(key), groupMin));
                stackGroupMaxMap.set(key, Math.max(stackGroupMaxMap.get(key), groupMax));
            }
        }
    }
    // Combine stack group min/max
    for (const [key, minVal] of stackGroupMinMap) {
        const maxVal = stackGroupMaxMap.get(key);
        const axis = key.split('_')[1];
        if (axis === 'y2') {
            if (minVal !== Number.MAX_VALUE) {
                y2DataMin = Math.min(y2DataMin, minVal);
            }
            if (maxVal !== -Number.MAX_VALUE) {
                y2DataMax = Math.max(y2DataMax, maxVal);
            }
        }
        else {
            if (minVal !== Number.MAX_VALUE) {
                yDataMin = Math.min(yDataMin, minVal);
            }
            if (maxVal !== -Number.MAX_VALUE) {
                yDataMax = Math.max(yDataMax, maxVal);
            }
        }
    }
    return {
 ...
 * @returns
 */
function getDataExtent(series, groups, getDataItem, xStartIndex, xEndIndex, isStacked, hiddenSet, hideAndShowBehavior, yAxis, y2Axis, getStackGroup) {
    ...
    // }
    const data = getDataMinMax(series, groups, xStartIndex, xEndIndex, getDataItem, isStacked, yAxis?.scale === 'log', hiddenSet, hideAndShowBehavior, getStackGroup);
    if (yDataMin == null) {
 ...
 * @param groups The groups array
 */
function hasData(series, groups) {
    return series.length > 0 && groups.length > 0;
}
/**
 ...
 * @param series The series array
 * @param groups The groups array
 * @param getDataItem The function that returns data item
 */
function hasInvalidData(timeAxisType, series, groups, getDataItem) {
    return hasInvalidTimeData(timeAxisType, series, groups, getDataItem);
}
/**
 * Returns true if there is a time axis and data doesn't have valid numerical data for it.
 * @param timeAxisType The time axis type
 * @param series The series array
 * @param groups The groups array
 * @param getDataItem The function that returns data item
 */
function hasInvalidTimeData(timeAxisType, series, groups, getDataItem) {
    if (timeAxisType === undefined) {
        return false;
    }
    ...
    if (timeAxisType === 'mixedFrequency') {
        // Check that all the x values are valid date strings
        for (let s = 0; s < series.length; s++) {
            for (let g = 0; g < groups.length; g++) {
                const item = getDataItem(s, g, [g]);
                if (item && (typeof item.x !== 'string' || isNaN(Date.parse(item.x)))) {
                    return true;
                }
            }
        }
        ...
        // Check that all group ids are valid date strings
        for (let i = 0; i < groups.length; i++) {
            const groupId = groups[i].id;
            if (isNaN(Date.parse(groupId))) {
                return true;
            }
        }
    }
...
function getDataAxisProps(position, availSpace, isRtl, locale, dataAxisInfo, dataAxis, axisFontSize, getTextDimensions, isAxisRendered) {
    ...
    const isHoriz = position === 'top' || position === 'bottom';
    const isSize = dataAxis?.size !== undefined;
    const axisMaxDim = getAxisMaxSize(availSpace, isHoriz, dataAxis?.size, dataAxis?.maxSize);
    ...
    const getLabelInfo = (tick, index, _) => {
        ...
        return getTickLabelInfo(coord, tick, index, tickLabelStyle, isHoriz, false, false, false, isHoriz ? maxLabelHeight : axisMaxDim - axisDim, isHoriz ? axisMaxDim - axisDim : maxLabelHeight, availSpace, getTextDimensions);
    };
    ...
    let labelDims = 0;
    if (isLabelRendered) {
        const maxW = getMaxWidth(isSize, position, axisMaxDim, maxWidth, titleDim);
        const maxH = getMaxHeight(isSize, position, axisMaxDim, maxHeight, titleDim);
        updateLabelsDims(maxW, maxH, tickLabels, getAxisPos, availSpace, position);
        labelDims = isHoriz ? maxH + AXIS_DEFAULTS.labelGap : maxW + AXIS_DEFAULTS.labelGap;
    }
 ...
 * @returns
 */
function getGroupAxisInfo(leafGroups, xAxis = {}, startOffset, endOffset, initialZooming, getTextDimensions, availSpace, isHoriz, zoomAndScroll) {
    const xAxisFontStyle = { ...xAxis?.tickLabel?.style };
    let { viewportMin, viewportMax, viewportEndGroup, viewportStartGroup } = xAxis;
    const groupCount = leafGroups.length;
    const numGroups = groupCount - 1;
    const globalMin = 0;
    const globalMax = numGroups;
    const { min, max } = applyInitialZooming({
        zoomAndScroll,
        initialZooming,
        currentMin: viewportMin,
        currentMax: viewportMax,
        globalMin,
        globalMax,
        startOffset,
        endOffset,
    ...
    viewportMin = min ?? viewportMin;
    viewportMax = max ?? viewportMax;
    if (viewportMin != null) {
...
function getGroupAxisProps(position, availSpace, isRtl, viewportMin, viewportMax, min, max, startIndex, endIndex, isDomainFlipped, levelsArray, endOffset, barWidths, isStacked, groupAxis, axisFontSize, getTextDimensions, isTestEnv, groupCount, hasWidthWeightValue) {
    ...
    const isHoriz = position === 'top' || position === 'bottom';
    const isSize = groupAxis?.size !== undefined;
    const axisMaxDim = getAxisMaxSize(availSpace, isHoriz, groupAxis?.size, groupAxis?.maxSize);
    ...
    for (let level = 0; level < levelsArray.length; level++) {
        const sIndex = getLevelStartingGroup(levelsArray, level, viewportMin, min, max, scale, isRtl, isHoriz);
        const eIndex = getLevelEndingGroup(levelsArray, level, viewportMax, min, max, scale, isRtl, isHoriz);
        ...
        const getLabelInfo = (label, index, isRotated, allowTruncate, isWrapAllowed) => {
            const rangeValue = getLabelCoord(level, index + sIndex, startIndex, endIndex, levelsArray, scale);
            return getTickLabelInfo(rangeValue, label, index + sIndex, tickLabelStyle, isHoriz, isRotated, isWrapAllowed, allowTruncate, isHoriz ? axisStepWidth : maxLabelDim, isHoriz ? maxLabelDim : axisStepWidth, availSpace, getTextDimensions);
        };
        const autoRotate = groupAxis?.tickLabel?.autoRotate != false;
        const levelInfo = getGroupAxisLabelsInfo(labelsInView, getLabelInfo, isHoriz, autoRotate, isWrapAllowed, numOfSafeSkips);
        ...
        if (isLabelRendered && isLevelRendered) {
            ...
            const maxH = getMaxHeight(isSize, position, axisMaxDim, maxHeight, titleDim);
            updateLabelsDims(maxW, maxH, tickLabels, getAxisPos, availSpace, position);
            const levelDims = isHoriz ? maxH + AXIS_DEFAULTS.labelGap : maxW + AXIS_DEFAULTS.labelGap;
            labelDims += levelDims;
    ...
    if (groupSeparators?.isRendered != false) {
        ...
        separators = getSeparatorPaths(levelsArray, separatorStartLevel, levelsInfo, isHoriz, scale, endOffset, isRtl);
    }
 ...
 * @returns
 */
function getLayoutInfo(series, groups, getDataItem, xAxis, yAxis, y2Axis, availSpace, hiddenSet, hideAndShowBehavior, isHoriz, isRtl, isStacked, axisFontSize, getStartAndEndAxesOffset, locale, splitDualY, splitterPositin, getStackGroup, getTextDimensions, isTestEnv, hasXDataAxis, initialZooming, zoomAndScroll) {
    ...
    if (hasXDataAxis) {
        dataInfo = getDataExtent(series, leafGroups, getDataItemWithGroupPath, 0, leafGroups.length - 1, isStacked, hiddenSet, hideAndShowBehavior, xAxis, undefined, getStackGroup);
        ...
        if (isTimeAxis) {
            ...
            xAxisInfo = getTimeAxisInfo(leafGroups, xAxis, startOffset, endOffset, timeAxisRange, isHoriz, availSpace, zoomAndScroll, initialZooming, getTextDimensions);
        }
        else {
            xAxisInfo = getGroupAxisInfo(leafGroups, xAxis, startOffset, endOffset, initialZooming, getTextDimensions, availSpace, isHoriz, zoomAndScroll);
        }
        dataInfo = getDataExtent(series, leafGroups, getDataItemWithGroupPath, xAxisInfo.startIndex, xAxisInfo.endIndex, isStacked, hiddenSet, hideAndShowBehavior, yAxis, y2Axis, getStackGroup);
    }
    ...
    const { axisDim: yDim } = getDataAxisProps(yAxisPosition, cloneDimension(availSpace), isRtl, locale, yAxisInfo, yAxis, axisFontSize, getTextDimensions, isYAxisRendered);
    const getYPreferredSize = (width, height) => {
        const availableSpace = getAvailSpace(width, height);
        const { axisDim: yDim } = getDataAxisProps(yAxisPosition, { width: availableSpace.width, height: availableSpace.height, x: 0, y: 0 }, isRtl, locale, yAxisInfo, yAxis, axisFontSize, getTextDimensions, isYAxisRendered);
        return { width: isHoriz ? width : Math.ceil(yDim), height: isHoriz ? Math.ceil(yDim) : height };
    };
    const { axisDim: y2Dim } = getDataAxisProps(y2AxisPosition, cloneDimension(availSpace), isRtl, locale, y2AxisInfo, y2Axis, axisFontSize, getTextDimensions, isY2AxisRendered);
    const getY2PreferredSize = (width, height) => {
        const availableSpace = getAvailSpace(width, height);
        const { axisDim: y2Dim } = getDataAxisProps(y2AxisPosition, { width: availableSpace.width, height: availableSpace.height, x: 0, y: 0 }, isRtl, locale, y2AxisInfo, y2Axis, axisFontSize, getTextDimensions, isY2AxisRendered);
        return {
            width: isHoriz ? width : Math.ceil(y2Dim),
            height: isHoriz ? Math.ceil(y2Dim) : height
        };
    };
    ...
    const getXPreferredSize = (width, height) => {
        const availableSpace = getAvailSpace(width, height);
        const axisProps = isTimeAxis
            ? getTimeAxisProps(leafGroups, xAxisPosition, { width: availableSpace.width, height: availableSpace.height, x: 0, y: 0 }, xAxisInfo.viewportMin, xAxisInfo.viewportMax, xAxisInfo.step, isRtl, xAxis, axisFontSize, getTextDimensions, isTimeAxis)
            : hasXDataAxis
                ? getDataAxisProps(xAxisPosition, xAvailSpace, isRtl, locale, xAxisInfo, xAxis, axisFontSize, getTextDimensions)
                : getGroupAxisProps(xAxisPosition, xAvailSpace, isRtl, xAxisInfo.viewportMin, xAxisInfo.viewportMax, xAxisInfo.min, xAxisInfo.max, xAxisInfo.startIndex, xAxisInfo.endIndex, isHoriz, levelsArray, endOffset, barWidths, isStacked, xAxis, axisFontSize, getTextDimensions, isTestEnv, groupCount, hasWidthWeightValue);
        return {
            width: isHoriz ? height : axisProps.axisDim + 2,
            height: isHoriz ? axisProps.axisDim + 2 : width
        };
    };
 ...
 * Returns the markertype order.
 */
function getLabelPosition(labelPosition, tx, ty, labelDims, isRtl) {
    const halfLabelWidth = labelDims?.width / 2;
    if (labelPosition === 'aboveMarker') {
        ...
        tx = isRtl ? tx - LABEL_OFFSET - halfLabelWidth : tx + LABEL_OFFSET + halfLabelWidth;
    }
...
function getActiveInfo(focusedItemInfo, hoveredItemInfo) {
    return focusedItemInfo?.isCurrent
        ? focusedItemInfo
...
export { getScrollBarSpace as a, getLayoutInfo as b, updateScrollBarSpace as c, hasData as d, getActiveInfo as e, findSeriesGroupIndexFromId as f, getAxesPosition as g, hasInvalidData as h, getAvailSpace as i, getOuterGaps as j, isNonCurvedLine as k, getLineAreaSegments as l, getCmdPoints as m, getBarGapRatio as n, findNearestDataPoint as o, getStartAndEndAxesOffset as p, getBarGapsInfo as q, getMixedFreqGroupCenter as r, getMarkers as s, getSeriesData as t, updateViewportFromOverview as u, getLabelPosition as v, getGroupCenterCoord as w, getMarkerFillAndStroke as x, getCumulativeSeries as y };
//# sourceMappingURL=utils-24c54d43.js.map
