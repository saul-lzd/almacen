...
import { ReferenceObjectData } from '../utils/UNSAFE_visTypes/chart';
export type BarItemData<K> = ChartItemData<K> & {
    ...
    value: number;
    /**
     * The time value of the data item. This property should only be set if xAxis.timeAxisType is set to mixedFrequency.
     */
    x?: string;
    /**
     * The weight used to determine the relative width of the bar.
     * Larger weight values, relative to other item weight values, will result in proportionally wider bars.
     */
    widthWeight?: number;
};
...
export type BarChartProps<K, D extends BarItemData<K>> = ChartCommonProps<K, D> & {
    ...
    /**
     * The stacking config for bar chart.
     */
    stacking?: 'off' | 'on' | StackConfig<K, D>;
};
export type StackConfig<K, D> = {
    /**
     * Defines how items are stacked in the group.
     * If defined, the series that returns the same string will be stacked together.
     * The series that returns undefined will be stacked together in a default stack group.
     */
    getStackGroup: (series: BarSeriesData<K, D>) => string | undefined;
    /**
