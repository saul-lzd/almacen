/**
 * @license
 * Copyright (c) %FIRST_YEAR% %CURRENT_YEAR%, Oracle and/or its affiliates.
 * Licensed under The Universal Permissive License (UPL), Vers.0
 * as shown at https://oss.oracle.com/licenses/upl/
 * @ignore
 */
import type { ComponentChildren } from 'preact';
import type { ButtonIntrinsicProps, ButtonIntrinsicAriaProps } from '../../utils/UNSAFE_buttonUtils';
import { TestIdProps } from '../UNSAFE_useTestId';
/**
 * The ActionDetail reasons
 */
type ActionDetail = {
    reason: 'pointer' | 'keyboard';
};
/**
 * Element details
 */
export type BareButtonElementDetails = {
    type: 'button';
    isFocusable?: never;
    buttonType?: 'button' | 'submit';
    isLabel?: never;
    isSwitch?: boolean;
} | {
    type: 'a';
    buttonType?: never;
    href: string;
    target?: string;
    rel?: string;
    isFocusable?: never;
    isLabel?: never;
    isSwitch?: never;
} | {
    type: 'span';
    buttonType?: never;
    isFocusable?: boolean;
    isLabel?: boolean;
    isSwitch?: boolean;
} | {
    type: 'div';
    buttonType?: never;
    isFocusable?: boolean;
    isLabel?: boolean;
    isSwitch?: boolean;
};
export type BareButtonProps = ButtonIntrinsicProps & ButtonIntrinsicAriaProps & TestIdProps & {
    /**
     *The default slot is the button's text label. The oj-c-base-button element accepts plain text or DOM nodes as children for the default slot."
     */
    children?: ComponentChildren;
    /**
     * Specifies that the button element should be disabled.
     */
    isDisabled?: boolean;
    /**
     * Specifies if repeated events are dispatched for key or pointer held down
     */
    isRepeat?: boolean;
    /**
     * Specifies if Button should use only a click handler, not keyup/down, ptr up/down
     */
    isClick?: boolean;
    /**
     * Triggered when a button is clicked, whether by keyboard, mouse, or touch events.
     */
    onAction?: (details: ActionDetail) => void;
    /**
     * Indicate element type and associated details
     */
    elementDetails?: BareButtonElementDetails;
};
/**
 * BareButtons are unformatted bare buttons that direct users to initiate or take actions and work with a single tap, click, or keystroke.
 */
export declare function useBareButton({ onAction, isDisabled, isRepeat, isClick, elementDetails, testId, ...props }: BareButtonProps): Record<string, any>;
export {};
