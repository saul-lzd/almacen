...
export type ScatterItem<K> = ChartItemData<K> & {
    /**
    ...
    markerSize?: number;
    /**
     * The data label of the data item.
     */
    label?: string;
    /**
     * The style object defining the style of the data label.  The following style properties are supported: color, fontFamily, fontSize, fontStyle, fontWeight.
     */
...
export type ScatterSeriesData<K, D> = Omit<ChartSeriesData<K, D>, 'associatedYAxis'> & {
    /**
...
export type ScatterChartProps<K, D extends ScatterItem<K>> = Omit<ChartCommonProps<K, D>, 'y2Axis' | 'initialZooming'> & {
    /**
     * The array of series for the chart.
     */
    series: ScatterSeriesData<K, D>[];
    /**
     * A function that returns a ScatterChartItem for each group and series.
     */
    getDataItem: (seriesIndex: number, groupIndex: number) => D | undefined;
    /**
     * The effect to add on data labels. If set to 'outline', data labels will have a contrast outline.
     */
