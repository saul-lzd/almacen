define(["require", "exports", "ojs/ojabortreason", "ojs/ojdataproviderfactory", "preact/hooks", "../PRIVATE_useEffectEvent/useEffectEvent"], function (require, exports, ojabortreason_1, ojdataproviderfactory_1, hooks_1, useEffectEvent_1) {
    "use strict";
     ...
     */
    const initialLoadingState = Object.freeze({
        status: 'loading'
    });
    ...
    function compareFilterCriterion(a, b) {
        if (a === b)
            return true;
    ...
    function compareSortCriteria(a, b) {
        if (a === b)
            return true;
        if (!a || !b || a.length !== b.length)
            return false;
        return a.every((crit, i) => crit.attribute === b[i].attribute && crit.direction === b[i].direction);
    }
    /**
    ...
    const useListData = (data, options = defaultOptions) => {
        const isInitialLoadRef = (0, hooks_1.useRef)(true);
        ...
        const [optionsState, setOptionsState] = (0, hooks_1.useState)({ data, options });
        const [state, setState] = (0, hooks_1.useState)(options.initialRowsFetched === 0 ? (0, exports.getEmptyState)('atLeast') : initialLoadingState);
        const effectiveFilterCriterionRef = (0, hooks_1.useRef)();
        const effectiveSortCriteriaRef = (0, hooks_1.useRef)();
        const isFilterUpdated = !(compareFilterCriterion(optionsState.options.filterCriterion, options.filterCriterion) ||
            (state.data != null &&
                compareFilterCriterion(state.data.filterCriterion, options.filterCriterion)));
        const isSortUpdated = !(compareSortCriteria(optionsState.options.sortCriteria, options.sortCriteria) ||
            (state.data != null && compareSortCriteria(state.data.sortCriteria, options.sortCriteria)));
        /**
        ...
        const fetchRange = (0, hooks_1.useCallback)(async (range, resultsCallback) => {
            ...
            if (range.count === 0) {
                ...
                setState({
                    ...
                    data: {
                        data: [],
                        filterCriterion: effectiveFilterCriterionRef.current,
                        offset: range.offset,
                        sizePrecision: sizePrecision,
                        sortCriteria: effectiveSortCriteriaRef.current,
            ...
            try {
                setState({ status: 'loading', data: state.data });
                ...
                setState({
                    ...
                    data: {
                        data: results,
                        filterCriterion: result.fetchParameters.filterCriterion,
                        offset: range.offset,
                        sizePrecision: sizePrecision,
                        sortCriteria: result.fetchParameters.sortCriteria,
                        totalSize: totalSizeRef.current
                    }
                });
            }
        ...
        }, [dataProvider, options.attributes, options.filterCriterion, options.sortCriteria, state.data]);
        /**
        ...
        const loadInitial = (0, hooks_1.useCallback)(async () => {
            ...
            try {
                ...
                effectiveFilterCriterionRef.current = fetchParameters?.filterCriterion;
                effectiveSortCriteriaRef.current = fetchParameters?.sortCriteria;
        ...
        }, [loadInitial, fetchRange, state.data]);
        const resetAndLoad = () => {
            // reset value of refs
            iteratorRef.current = null;
            fetchNextRef.current = null;
            ...
            effectiveFilterCriterionRef.current = undefined;
            effectiveSortCriteriaRef.current = undefined;
            setOptionsState({ data, options });
            isInitialLoadRef.current = false;
            if (options.initialRowsFetched === 0) {
                setState((0, exports.getEmptyState)('atLeast'));
            }
        ...
        const STABLE_handleMutation = (0, useEffectEvent_1.useEffectEvent)((event) => {
        ...
        const STABLE_handleRefresh = (0, useEffectEvent_1.useEffectEvent)((event) => {
            let adjustment = -1;
        ...
        // handles refresh and mutation events from DataProvider
        (0, hooks_1.useEffect)(() => {
            if (dataProvider) {
                dataProvider.addEventListener('refresh', STABLE_handleRefresh);
                dataProvider.addEventListener('mutate', STABLE_handleMutation);
            }
            return () => {
                if (dataProvider) {
                    dataProvider.removeEventListener('refresh', STABLE_handleRefresh);
                    dataProvider.removeEventListener('mutate', STABLE_handleMutation);
                }
            };
        }, [STABLE_handleMutation, STABLE_handleRefresh, dataProvider]);
        // runs on every render but only takes action on component mount and and when the current
        // iterator no longer satisfies the options currently specified
        (0, hooks_1.useEffect)(() => {
            if (dataProvider &&
                (isInitialLoadRef.current ||
                    optionsState.data !== data ||
                    optionsState.options.attributes !== options.attributes ||
                    optionsState.options.fetchSize !== options.fetchSize ||
                    optionsState.options.initialRowsFetched !== options.initialRowsFetched ||
                    optionsState.options.isInitialFetchDeferred !== options.isInitialFetchDeferred ||
                    isFilterUpdated ||
                    isSortUpdated)) {
                resetAndLoad();
            }
        });
        // abort any outstanding fetches when component unmounts
        (0, hooks_1.useEffect)(() => {
            return () => {
                abortControllerRef.current?.abort((0, ojabortreason_1.getAbortReason)());
            };
    ...
    const handleAddRemoveMutation = (detail, dataState, options, isAdd) => {
        ...
        if (isIndexesAvailable(detail, options)) {
            ...
            if (isAdd) {
                if (detail.addBeforeKeys) {
                    // only add before the keys that are already fetched
                    const allKeys = getAllKeys(dataState);
                    detail.addBeforeKeys.forEach((key) => {
                    ...
                    // if no indexes or addBeforeKeys, assume the items are added at the end
                    itemCount += detail.keys.size;
                }
            }
            else {
    ...
    /**
     * A helper function to get all the keys from dataState
     * @param dataState
     */
    const getAllKeys = (dataState) => {
        return dataState.data.map((d) => d.metadata.key);
    };
});
